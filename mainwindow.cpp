...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto race1 = [&]{return concurRace1->DoWork(&number, &m, ui->sb_initNum->value());};
        auto race2 = [&]{return concurRace2->DoWork(&number, &m, ui->sb_initNum->value());};

        QFuture result = QtConcurrent::run(race1).then(race2);
    }
    else{
